...
export class RerankClientService {
  ...
  async rerank(
    ...
    if (!res.ok) {
      this.logger.error(
        'RerankClientService.rerank: Cohere API call failed, falling back to unreranked',
        {
          projectId,
          status: res.status,
        },
    ...
    this.logger.log('RerankClientService.rerank: done', {
      projectId,
      rerankedCount: body.results.length,
    });
    return {
      chunks: body.results.map((r) => candidates[r.index]),
      usedCohere: true,
    };
  }
}
