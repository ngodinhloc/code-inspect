...
export class EmbeddingClientService {
  ...
  async embed(text: string, projectId: string): Promise<number[]> {
    this.logger.log('EmbeddingClientService.embed: starting', { projectId, textLength: text.length });
    ...
    if (!res.ok) {
      this.logger.error('EmbeddingClientService.embed: request failed', { projectId, status: res.status });
      throw new Error(`Embedding service returned ${res.status}`);
    }
    const body = (await res.json()) as { embeddings: number[][] };
    this.logger.log('EmbeddingClientService.embed: done', { projectId, dimensions: body.embeddings[0].length });
